...
bcftools merge -m id --regions ${region_array} ${workdir}/03c_vcf/Cat_*.vcf.gz > \
${workdir}/04c_vcf/Cat_${region_array}.vcf
...
vcftools --vcf ${workdir}/04c_vcf/Cat_${region_array}.vcf --keep keep_cat.txt --thin 10000 \
--max-missing 0.8 --mac 4 --max-alleles 2 --max-maf 0.49 --recode \
--recode-INFO-all --out ${workdir}/05c_pca/Cat_structure_${region_array}
...
vcftools --vcf ${workdir}/04c_vcf/Cat_${region_array}.vcf --keep keep_cat.txt \
--max-missing 0.65 --max-alleles 2 \
--max-maf 0.49 --remove-indels --recode --recode-INFO-all \
--out ${workdir}/06c_stats_cat/${region_array}
...
bgzip ${workdir}/06c_stats_cat/${region_array}.recode.vcf
...
tabix -p vcf ${workdir}/06c_stats_cat/${region_array}.recode.vcf.gz
...
vcftools --vcf ${workdir}/04c_vcf/Cat_${region_array}.vcf --keep keep_cat.txt \
--max-missing 0.65 --mac 2 --max-alleles 2 --max-maf 0.49 --recode \
--recode-INFO-all --out ${workdir}/07c_transversions/Cat_${region_array}
...
grep "^#" ${workdir}/07c_transversions/Cat_${region_array}.recode.vcf > \
${workdir}/07c_transversions/Cat_${region_array}.subset.vcf
...
Rscript _extract_transversions.r ${workdir}/07c_transversions/Cat_${region_array}.recode.vcf \
${workdir}/07c_transversions/Cat_${region_array}.subset.vcf
...
rm ${workdir}/07c_transversions/Cat_${region_array}.recode.vcf
